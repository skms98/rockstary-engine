...
export default function AttractionsDashboard() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const entry = {
      ...
      keywords_list: formData.keywords_list || '',
    ...
    if (!error) {
      setShowNewForm(false)
      setFormData({ event_id: '', event_title: '', event_url: '', raw_text: '', keywords_list: '', screenshot_file: null })
      loadEntries()
    }
    setSubmitting(false)
  }
  ...
  async function massExport(format: 'xlsx' | 'csv') {
    ...
    if (format === 'csv') {
      // CSV: flat pipeline format for Google Sheets
      const headers = [
    ...
    // XLSX: Reference format â one sheet per attraction with columns A-D-E-F
    for (const entry of toExport) {
      ...
      rows.push(['Keywords can be combined with each other', 'Section 1 - Headline / H1', entry.event_title, ' ',
        entry.categories ? JSON.stringify({ final_status: 'VALID', domain: 'ATTRACTION', primary_category: entry.categories.split(',')[0]?.trim(), tags: entry.tags?.split(',').map((t: string) => t.trim()) || [] }, null, 2) : null,
        entry.page_qa_comments || null])
      rows.push([entry.event_url, null, null, null, null, null])
      rows.push([`keywords to include: \n\n${entry.keywords_list || ''}`, 'Section 2 - Teaser', null, sections['TEASER'] || null])
      rows.push([null, 'Section 3 - Icons', null, null])
      rows.push([null, 'Section 5', 'What To Expect', 'What To Expect'])
      rows.push([null, null, entry.original_description, sections['WHAT TO EXPECT'] || null])
      rows.push([null, null, null, sections['CTA'] || null])
      rows.push([null, 'Section 6', 'Highlights', 'Highlights'])
      rows.push([null, null, null, sections['HIGHLIGHTS'] || null])
      rows.push([null, null, null, null])
      rows.push([null, 'Section 7 ', "What\u2019s included in the ticket", "What\u2019s included in the ticket"])
      rows.push([null, null, null, sections['INCLUSIONS'] || null])
      rows.push([null, null, 'Exclusions', 'Exclusions'])
      rows.push([null, null, null, sections['EXCLUSIONS'] || null])
      rows.push([null, 'Section 8', 'Timing: ', 'Timing: '])
      rows.push([null, null, null, sections['TIMINGS'] || null])
      rows.push([null, 'Section 9', 'Ticket Information: ', 'Ticket Information: '])
      rows.push([null, null, 'the price will be displayed automatically', 'the price will be displayed automatically'])
      rows.push([null, 'Section 10', 'Important things to know before your visit', 'Important things to know before your visit'])
      rows.push([null, null, null, sections['IMPORTANT THINGS TO KNOW'] || null])
      rows.push([null, null, 'Cancelation policy: ', 'Cancelation policy: '])
      rows.push([null, null, null, sections['CANCELLATION POLICY'] || null])
      rows.push([null, 'Section 10 - Location + How To Get There', 'Address:', 'Address:'])
      rows.push([null, null, null, null])
      rows.push([null, null, 'How to get there:', 'How to get there:'])
      ...
      rows.push([null, null, 'By car: ', 'By car: '])
      rows.push([null, null, null, byCarMatch ? byCarMatch[1].trim() : null])
      rows.push([null, null, 'By taxi: ', 'By taxi: '])
      rows.push([null, null, null, byTaxiMatch ? byTaxiMatch[1].trim() : null])
      rows.push([null, null, 'By public transportation: ', 'By public transportation: '])
      rows.push([null, null, null, byPublicMatch ? byPublicMatch[1].trim() : null])

      const ws = XLSX.utils.aoa_to_sheet(rows)
      ws['!cols'] = [{ wch: 45 }, { wch: 35 }, { wch: 50 }, { wch: 50 }, { wch: 40 }, { wch: 40 }]
      const sheetName = (entry.event_title || `Attraction ${entry.event_id}`).substring(0, 31)
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-pl-navy border border-pl-border rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              {/* Keywords List (available for all non-excel input methods) */}
              {inputMethod !== 'excel_upload' && (
                <div>
                  <label className="block text-sm text-pl-text-dim mb-2">Keywords List <span className="text-emerald-400 text-xs">(optional, paste numbered keywords)</span></label>
                  <textarea
                    value={formData.keywords_list}
                    onChange={(e) => setFormData({ ...formData, keywords_list: e.target.value })}
                    placeholder={"1- attraction name\n2- city\n3- tickets\n4- price\n5- experience type\n..."}
                    className="pl-input min-h-[120px] resize-y font-mono text-sm"
                  />
                  <p className="text-xs text-pl-muted mt-1">If provided, keywords will be used for SEO optimization with (keyword) [number] annotations. If blank, AI will generate them.</p>
                </div>
              )}
