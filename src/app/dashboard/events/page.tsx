...
export default function EventsDashboard() {
  ...
  async function processAllWithAI() {
    ...
    if (!confirm(`Process ${withDescription.length} event${withDescription.length > 1 ? 's' : ''} through all AI steps? This runs in the background â you can navigate away.`)) return
    ...
    // Fire the request â it runs server-side, but we poll for updates
    try {
  ...
  async function singleExportCSV(entry: any) {
    ...
    const row = [
      ...
      entry.original_description, entry.recommended_versions, entry.fact_check_scores,
      entry.duplicate_analysis, entry.ab_tests, entry.organiser_trigger_risk, entry.tov_score,
      entry.grammar_style, entry.reviewer_output, entry.resolver_output, entry.prev_original_description,
      entry.seo_analysis, entry.fact_check_final, entry.ranked_versions, entry.status
    ]
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `event-${entry.event_id || entry.id}.csv`
    a.click()
    URL.revokeObjectURL(url)
  }
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-medium text-sm disabled:opacity-40 disabled:cursor-not-allowed transition-all"
      ...
      {processAllStatus && (
        <div className={`mb-6 rounded-xl border p-4 ${processAllStatus.running ? 'bg-indigo-600/10 border-indigo-500/30' : processAllStatus.errors > 0 ? 'bg-yellow-600/10 border-yellow-500/30' : 'bg-green-600/10 border-green-500/30'}`}>
          ...
          <div className="w-full h-2 bg-pl-dark rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${processAllStatus.running ? 'bg-indigo-500 animate-pulse' : 'bg-green-500'}`}
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-3">
                  {[
                    ...
                    { value: 'url_only' as InputMethod, icon: 'ð', label: 'URL Only', desc: 'Just provide the event URL' },
          ...
          {entries.map((entry) => (
            <div key={entry.id} className="pl-card flex items-center group">
              ...
              <button
                ...
                className="p-3 rounded-lg text-pl-muted/40 hover:text-pl-gold hover:bg-pl-gold/10 transition-all opacity-0 group-hover:opacity-100"
